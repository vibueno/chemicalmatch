...
window.onload = function() {
  ...
  chemMatchGame.deck.DOMNode.addEventListener('click', async function(event) {
    if (
      chemMatchGame.started &&
      chemMatchGame.roundComplete === false &&
      event.target.tagName === 'LI'
    ) {
      let chemMatchCard = chemMatchGame.deck.cards.find(
        card => 'card-' + card.id === event.target.id
      );
      ...
      if (chemMatchGame.isRoundComplete()) {
        ...
        if (chemMatchGame.isGameSolved()) {
          chemMatchGame.end();
          let dialogText = `You solved the game in ${
            chemMatchGame.moveCounter.moves
          } moves,
                  ${functions.formatMinutes(
                    chemMatchGame.timer.seconds
                  )} minute(s) and
